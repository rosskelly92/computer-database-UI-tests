...
Feature: Create
  ...
  @smoke @create @TC002
  Scenario: 01 - Add a new computer with minimum mandatory data
  ...
  @smoke @create @TC001
  Scenario: 02 - Add a new computer with all possible data
    When I add a new computer with all fields populated
    Then the save message reads "Done! Computer " computer name " has been created"

  @smoke @read @TC001
  Scenario: 03 - Computer details are displayed correctly in table and edit mode
  ...
  @create @TC003
  Scenario: 04 - Cannot add a computer without a name field
  ...
  @create @TC007
  Scenario: 05 - Cancel addition
    When I cancel a new computer addition
    And I search for the computer
    Then there are no search results

  @create @TC004 @bug @B002
  Scenario Outline: 06 - Add computer - date validation on Introduced field
  ...
  Scenario Outline: 07 - Add computer - date validation on Discontinued field
  ...
  Scenario: 08 - add mode - special characters in Name field
