...
Feature: Create
  ...
  @retrieve @TC020
  Scenario: 01 - On page 1 of search results, Previous button is disabled
    Then previous button is disabled

  @retrieve @TC020
  Scenario: 02 - Navigation between pages of search results
    When I click next page
    Then I am on the second page of results
    And I can navigate back to the first page

  @retrieve @TC020
  Scenario: 03 - Correct number of search results retrieved
  ...
  @retrieve @TC021
  Scenario: 04 - Clear filter returns all results
  ...
  @retrieve @TC022
  Scenario: 05 - Change filter criteria
  ...
  Scenario: 06 - Search results span multiple pages
  ...
  Scenario Outline: 07 - Verify Sort Order
